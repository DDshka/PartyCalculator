...
# todo instead of files we should use classes for services
...
# todo what does request object actually do here
...
def is_party_admin(request, party_id: int) -> bool:
    profile = get_profile_by_request(request)
    party = get_party_by_id(party_id)
    return Membership.objects.filter(profile=profile, party=party,
                                     is_owner=True).exists() or party.created_by == profile
